...
import type { ICellContent, IGridActions, IGridState, IMergeInfo } from "../types/grid";
import {
...
export const useGridState = (): IGridState & IGridActions => {
    ...
    const [mergedCells, setMergedCells] = useState<Map<string, IMergeInfo>>(new Map());
    ...
    const [defaultSlotDuration, setDefaultSlotDuration] = useState(DEFAULT_SLOT_DURATION);
    const [editingDefaultDuration, setEditingDefaultDuration] = useState(false);
    const [tempDefaultDuration, setTempDefaultDuration] = useState(String(DEFAULT_SLOT_DURATION));
    const [columnDurations, setColumnDurations] = useState<{ [key: number]: number }>({});
    const [cellContents, setCellContents] = useState<Map<string, ICellContent>>(new Map());
    ...
    const mergeCells = () => {
        ...
        for (let r = minRow; r <= maxRow; r++) {
            for (let c = minCol; c <= maxCol; c++) {
                ...
                if (cellKey !== masterCell) newHiddenCells.add(cellKey);
            }
        }
    ...
    const addColumnAfter = (afterColumnIndex: number) => {
        ...
        selectedCells.forEach((cellKey: string) => {
            ...
            newSelectedCells.add(getCellKey(row, col > afterColumnIndex ? col + 1 : col));
        });
        ...
        mergedCells.forEach((mergeInfo, cellKey) => {
            const [row, col] = cellKey.split("-").map(Number);
            const newCellKey = col > afterColumnIndex ? getCellKey(row, col + 1) : cellKey;
            newMergedCells.set(newCellKey, mergeInfo);

            const [mergeRow, mergeCol] = newCellKey.split("-").map(Number);
            for (let r = mergeRow; r < mergeRow + mergeInfo.rowSpan; r++) {
                for (let c = mergeCol; c < mergeCol + mergeInfo.colSpan; c++) {
                    ...
                    if (hiddenKey !== newCellKey) newHiddenCells.add(hiddenKey);
                }
            }
        });
    ...
    const deleteColumn = (columnIndex: number) => {
        ...
        selectedCells.forEach((cellKey: string) => {
            const [row, col] = cellKey.split("-").map(Number);
            if (col < columnIndex) newSelectedCells.add(cellKey);
            else if (col > columnIndex) newSelectedCells.add(getCellKey(row, col - 1));
        });
        ...
        mergedCells.forEach((mergeInfo, cellKey) => {
            ...
            const newCellKey = col > columnIndex ? getCellKey(row, col - 1) : cellKey;
            newMergedCells.set(newCellKey, mergeInfo);
        ...
        Object.keys(columnDurations).forEach((key: string) => {
            const index = parseInt(key);
            if (index < columnIndex) newColumnDurations[index] = columnDurations[index];
            else if (index > columnIndex) newColumnDurations[index - 1] = columnDurations[index];
        });
    ...
    const saveDurationEdit = () => {
        if (editingDuration !== null) {
            ...
            if (newDuration > 0 && newDuration <= 480) {
                setColumnDurations((prev) => ({ ...prev, [editingDuration]: newDuration }));
            }
        }
    ...
    const saveDefaultDurationEdit = () => {
        ...
        if (newDuration > 0 && newDuration <= 480) setDefaultSlotDuration(newDuration);
    ...
    const setAllMergedCells = (newMergedCells: Map<string, IMergeInfo>) => {
        setMergedCells(newMergedCells);
    };
